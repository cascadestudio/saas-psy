 ...
 */
function calculateScore(questionnaire, answers) {
 ...
 * Simple sum scoring
 */
function calculateSumScore(questionnaire, answers) {
    const totalScore = Object.values(answers).reduce((sum, value) => sum + (Number(value) || 0), 0);
    const interpretation = getInterpretation(totalScore, questionnaire.scoring?.ranges || []);
    return {
        totalScore,
        interpretation,
    };
 ...
 */
function calculateBDIScore(questionnaire, answers) {
    const totalScore = Object.values(answers).reduce((sum, value) => sum + (Number(value) || 0), 0);
    let severity = '';
    if (totalScore <= 13)
        severity = 'Dépression minimale';
    else if (totalScore <= 19)
        severity = 'Dépression légère';
    else if (totalScore <= 28)
        severity = 'Dépression modérée';
    else
        severity = 'Dépression sévère';
    return {
        totalScore,
        severity,
 ...
 */
function calculateSTAIScore(questionnaire, answers) {
    ...
    const stateScore = stateQuestions.reduce((sum, q) => sum + (Number(answers[q]) || 0), 0);
    const traitScore = traitQuestions.reduce((sum, q) => sum + (Number(answers[q]) || 0), 0);
    return {
        stateScore,
        traitScore,
 ...
 */
function calculateDualScaleScore(questionnaire, answers) {
    let anxietyScore = 0;
 ...
 */
function getInterpretation(score, ranges) {
